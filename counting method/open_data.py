...
class get_data(object):
    def __init__(self):
        ...
        self.train_data_path = "./crowd dataset/mall_dataset/frames/"
        self.ground_data_path = "./crowd dataset/mall_dataset/ground mall/"
        self.test_data_path = self.train_data_path
        self.test_ground_path = self.ground_data_path
    ...
    def get_train_data(self, down_size=1):
        ...
        for img_num in batch_list:
            num_convert = "%06d" %img_num
            img = skimage.io.imread(self.train_data_path+'seq_'+num_convert+'.jpg')
            
            width = img.shape[0]
            height = img.shape[1]
            #img = skimage.transform.resize(img, (width//2, height//2, 3))
            ...
            #ground = skimage.transform.resize(ground, (width//2, height//2))
            
            #ground = ground[rand_area_x:rand_area_x+self.size_x,rand_area_y:rand_area_y+self.size_y]
            if down_size != 1:
    ...
    def get_test_data(self, down_size=1, img_num=1):
        ...
        num_convert = "%06d" %img_num
        img = skimage.io.imread(self.test_data_path+"seq_"+num_convert+'.jpg')
        test_x = img.shape[0]
        test_y = img.shape[1]
