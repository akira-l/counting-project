...
def train():
    loss_file = open(r'fcn_train_loss.txt', 'a')
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
        ...
        for i in range(1, ts_num):
            ...
            if i % 100 == 0:
                if os.path.isdir('./fcn_save'):
                    saver.save(sess, './fcn_save/session2.ckpt')
                else:
                    os.mkdir('./fcn_save')
        loss_file.close()
...
def test():
    loss_file = open(r'fcn_test_loss.txt','a')
    data = get_data()
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
        ...
        if os.path.exists('./fcn_save/session2.ckpt.index'):
            saver = tf.train.Saver()
            saver.restore(sess, './fcn_save/session2.ckpt')
