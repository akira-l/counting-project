...
class Source_Data(object):
    ...
    def get_train_data(self): 
        ...
        for i in img_name_list:
            ...
            counter = 0
            for j in self.data_cap_map:
                counter+=1
                if counter > 2:
                    rand_para = random.randint(-self.random_value4box,self.random_value4box)
                else:
                    rand_para = 0
                img_list.append(img[j[2]:j[3],(j[0]+rand_para):(j[1]+rand_para),:])
                ground_list.append(ground[j[2]:j[3],(j[0]+rand_para):(j[1]+rand_para)])
                score_list.append(sum(sum(ground[j[2]:j[3],(j[0]+rand_para):(j[1]+rand_para)])))
                frame_1.append(frame1[j[2]:j[3],(j[0]+rand_para):(j[1]+rand_para),:])
                frame_2.append(frame2[j[2]:j[3],(j[0]+rand_para):(j[1]+rand_para),:])
        j = 0
        i = 0
