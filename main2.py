...
########################################################
'''
def get_resnet_output(data, initial_load = False):
    ...
    if initial_load is True:
        ...
        with res_graph.as_default():
            out1 = res_graph.get_tensor_by_name("scale1/Relu:0")
            out2 = res_graph.get_tensor_by_name("scale2/block3/Relu:0")
            out3 = res_graph.get_tensor_by_name("scale3/block4/Relu:0")
            #tf.get_variable_scope().reuse_variables()
            images = res_graph.get_tensor_by_name("images:0")
        with tf.Session(graph = res_graph) as sess:
...
with tf.Graph().as_default() as g1:
    saver = tf.train.import_meta_graph("/home/yuanzhi/figure/mexp/version2/resnet-pretrained/ResNet-L101.meta")
    out1 = g1.get_tensor_by_name("scale1/Relu:0")
    out2 = g1.get_tensor_by_name("scale2/block3/Relu:0")
    out3 = g1.get_tensor_by_name("scale3/block4/Relu:0")
    #tf.get_variable_scope().reuse_variables()
    images = g1.get_tensor_by_name("images:0")
with tf.Session(graph = g1) as sess:
    saver.restore(sess,"/home/yuanzhi/figure/mexp/version2/resnet-pretrained/ResNet-L101.ckpt")
    data_x1, data_x2, data_x3 = sess.run([out1, out2, out3], feed_dict={images:train_batch})
'''
...
with tf.Session(graph=g2) as sess:
    ...
    for i in range(1,ts_num+1):
        ...
        feed_dict={net.x_1:data_x1, net.x_2:data_x2, net.x_3:data_x3, net.ys:ground_batch, net.training_flag:True}
        ...
        if i%get_test_loss==0:
            ...
            feed_dict = {net.x_1:tdata_x1, net.x_2:tdata_x2, net.x_3:tdata_x3,net.ys:test_ground,net.training_flag:False}
            ...
            if i%record_test_map==0:
                feed_dict = {net.x_1:tdata_x1, net.x_2:tdata_x2, net.x_3:tdata_x3, net.training_flag:False}
                test_prediction = sess.run(prediction_img,feed_dict=feed_dict)
                stitching(test_prediction,i)
            '''
            test_, test_ground = get_test_data(215)
            tdata_x1, tdata_x2, tdata_x3 = resnet_.get_resnet_output(test_)
            feed_dict = {net.x_1:tdata_x1, net.x_2:tdata_x2, net.x_3:tdata_x3, net.ys:test_ground, net.training_flag:False}
            test_loss,lr_record_ = sess.run([net.loss_sum,learning_rate],feed_dict=feed_dict)
            ...
            test_, test_ground= get_test_data(210)
            tdata_x1, tdata_x2, tdata_x3 = resnet_.get_resnet_output(test_)
            feed_dict = {net.x_1:tdata_x1, net.x_2:tdata_x2, net.x_3:tdata_x3, net.ys:test_ground, net.training_flag:False}
            test_loss,lr_record_ = sess.run([net.loss_sum,learning_rate],feed_dict=feed_dict)
            lr_record.append(lr_record_)
