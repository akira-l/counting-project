...
def Global_Average_Pooling(x):
    ...
    return tf.reduce_mean(x, [1,2], name='Global_avg_pooling')
...
class Model_net(object):
    ...
    def model(self, x_input1, x_input2, x_input3, x_frame):
        ...
        with tf.variable_scope('model1'):
            ...
            x_squeeze = self.squeeze_excitation_layer(h_conv2, out_dim = 8, ratio = 2, layer_name = 'squeeze_layer_')
            
            
            W_conv3 = self.weight_variable([5,5,8,1])
            b_conv3 = self.biases_variable([1])
            h_conv3 = tf.nn.relu(tf.nn.conv2d(x_squeeze,W_conv3,strides=[1,1,1,1],padding='SAME')+b_conv3)
            
            
        with tf.variable_scope('model2'):
            #model2
            W2_conv1 = self.weight_variable([3,3,256,64])
            b2_conv1 = self.biases_variable([64])
            ...
            W2_conv2 = self.weight_variable([5,5,64,8])
            b2_conv2 = self.biases_variable([8])
            h2_conv2 = tf.nn.relu(tf.nn.conv2d(h2_pool1,W2_conv2,strides=[1,1,1,1],padding='SAME')+b2_conv2)

            x2_squeeze = self.squeeze_excitation_layer(h2_conv2, out_dim = 8, ratio = 2, layer_name = 'squeeze_layer_')

            W2_conv3 = self.weight_variable([5,5,8,1])
            b2_conv3 = self.biases_variable([1])
            h2_conv3 = tf.nn.relu(tf.nn.conv2d(x2_squeeze,W2_conv3,strides=[1,1,1,1],padding='SAME')+b2_conv3)
        ...
        with tf.variable_scope('model3'):
            ...
            x3_squeeze = self.squeeze_excitation_layer(h3_conv2, out_dim = 8, ratio = 2, layer_name = 'squeeze_layer_')
            
            W3_conv3 = self.weight_variable([5,5,8,1])
            b3_conv3 = self.biases_variable([1])
            h3_conv3 = tf.nn.relu(tf.nn.conv2d(x3_squeeze,W3_conv3,strides=[1,1,1,1],padding='SAME')+b3_conv3)
        ...
        with tf.variable_scope('delta_frame'):
            ...
            WF_conv1 = self.weight_variable([3,3,6,6])
            bF_conv1 = self.biases_variable([6])
            hF_conv1 = tf.nn.relu(tf.nn.conv2d(x_frame, WF_conv1, strides=[1,1,1,1], padding='SAME')+bF_conv1)
            hF_conv1 = tf.nn.max_pool(hF_conv1,ksize=[1,8,8,1],strides=[1,8,8,1],padding='SAME')

            xf_squeeze = self.squeeze_excitation_layer(hF_conv1, out_dim = 6, ratio = 1, layer_name = 'squeeze_layer_')

            WF_conv2 = self.weight_variable([5,5,6,1])
            bF_conv2 = self.biases_variable([1])
            hF_conv2 = tf.nn.relu(tf.nn.conv2d(xf_squeeze, WF_conv2, strides=[1,1,1,1], padding='SAME')+bF_conv2)
        ...
        with tf.variable_scope('merge_model'):
            ...
            node.append(h3_conv3)
            node.append(hF_conv2)
